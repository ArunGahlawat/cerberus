...
public class AllureRestAssuredMaskedHeadersFilter extends AllureRestAssured {
    public LogConfig logConfig;
    public String requestTemplatePath = "http-request.ftl";
    public String responseTemplatePath = "http-response.ftl";
    public String responseAttachmentName;

    public AllureRestAssuredMaskedHeadersFilter(LogConfig logConfig) {
    ...
    private Map<String, String> toMapConverterForHeaders(final Iterable<? extends NameAndValue> items) {
        final Map<String, String> result = new HashMap<>();
        String[] blackListHeaders = logConfig.blacklistedHeaders().toArray(new String[0]);
        items.forEach(existingHeader -> {
            if (StringUtils.equalsAnyIgnoreCase(existingHeader.getName(), blackListHeaders)) {
                result.put(existingHeader.getName(), "************");
            } else
